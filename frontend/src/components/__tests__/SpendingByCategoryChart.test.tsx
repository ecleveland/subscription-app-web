...
describe('SpendingByCategoryChart', () => {
  it('should render without crashing with valid subscription data', () => {
    const subs = [
      ...
      makeSub({ _id: '2', name: 'GitHub', cost: 4, category: 'Software' }),
    ];

    const { container } = render(<SpendingByCategoryChart subscriptions={subs} />);
    expect(container.firstChild).toBeTruthy();
  });

  it('should handle empty subscriptions array', () => {
    render(<SpendingByCategoryChart subscriptions={[]} />);
  ...
  it('should show empty state when all subscriptions are inactive', () => {
    ...
    render(<SpendingByCategoryChart subscriptions={subs} />);
